...
const types = ["Debug", "Release", "MinSizeRel", "RelWithDebInfo"];
...
function resolve(name, root) {
  ...
  if (!fs.existsSync) throw moduleError(name);

  if (path.extname(name) !== ".node") name += ".node";
  ...
  if (isPath(name)) {
    ...
    if (!fs.existsSync(file)) throw moduleError(file);
  ...
  for (;;) {
    ...
    if (fs.existsSync(build)) {
      ...
      for (const type of types) files.push(path.join(build, type, name));
...
function load(name, root) {
  ...
  if (load.cache[file]) return load.cache[file];
...
function isPath(str) {
  if (process.platform === "win32") str = str.replace("\\", "/");

  return str[0] === "/" || str.startsWith("./") || str.startsWith("../");
}
